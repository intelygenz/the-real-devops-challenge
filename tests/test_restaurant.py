...
def mock_find_restaurant(mongo, id=None):
    return [restaurant for restaurant in TEST_DATA if restaurant.get('_id') == ObjectId(id)]
...
class TestRestaurant(TestCase):
    ...
    @patch('src.mongoflask.find_restaurants')
    def test_mocked_get_resturant_returns_a_mocked_unique_element(self, mock_restaurant):
        mock_restaurant.side_effect = mock_find_restaurant
        data = src.mongoflask.find_restaurants(None, "55f14312c7447c3da7051b39")
    ...
    def test_get_resturant_returns_a_unique_element(self):
        mongo_client = pymongo.MongoClient('localhost')
        mongo_client.db.restaurant.insert_many(TEST_DATA)
        data = src.mongoflask.find_restaurants(mongo_client, "55f14312c7447c3da7051b39")
        self.assertTrue(data[0].get("type_of_food") == "Chinese")
